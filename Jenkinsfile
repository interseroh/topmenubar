node {
    ...
    stage('Build') {
        println "BRANCH: $GIT_BRANCH"
        // Run the maven build
        sh "'${mvnHome}/bin/mvn' clean deploy -Pwith-docker-release -Pwith-profile -Pwith-usermgt -DaltDeploymentRepository=nexus::default::http://nexus/repository/releases"
    }
    stage('Results') {
