...
public class UserInformationServiceImpl implements UserInformationService {
                ...
                public void run() {
                    ...
                    if (logoutUrl != null && !logoutUrl.trim().isEmpty()) {
                        if (ssoLogoutUrl != null && !ssoLogoutUrl.isEmpty()) {
                            String logoutHook = ssoLogoutUrl + "/endsession?post_logout_redirect_uri=" + logoutUrl;
                ...
                public void run() {
                    ...
                    userInfoClient.logout(new MethodCallback<Void>() {
                        ...
                        public void onFailure(Method method, Throwable exception) {
                            logger.warning("GW LOGOUT FAILED " + exception.getMessage());
                            methodObject3.run();
                        }
            ...
            final LogoutTask methodObject1 = new LogoutTask(new UserInformationServiceImpl(this)) {
                ...
                public void run() {
                    ...
                    userInfoClient.deepLogout(logoutUrl, new MethodCallback<Void>() {
                        @Override
                        public void onFailure(Method method, Throwable exception) {
                            logger.warning("DEEP LOGOUT FAILED " + exception.getMessage());
                            methodObject2.run();
                        }
    ...
    private static abstract class LogoutCallback implements Callback {
        ...
        private LogoutCallback(UserInformationServiceImpl userService, Callback nestedCallback) {
            this.userService = userService;
